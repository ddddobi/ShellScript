...
PkgInstall() {  #출력은 따로 없음
    PKG=$*
    yum -q -y install $PKG
    rpm -q $PKG >/dev/null 2>&1
    if [ $? -eq 0 ]; then
        echo "[ OK ] 패키지($PKG) 설치가 완료 되었습니다."
    else 
        echo "[ FAIL ] 패키지($PKG) 설치가 실패 하였습니다."
        exit 2
    fi
}  
...
MakeIndex() {
    ...
    if [ -f "$INDEX" ]; then
        echo "[ OK ] $INDEX 파일이 생성 되었습니다."
    else
        echo "[ FAIL ] $INDEX 파일이 생성에 실패했습니다."
        exit 3
    fi 
}
...
SvcStart() {
    ...
    if [ "$SVCSTATUS" = 'active' ]; then
        ...
        echo "[ FAIL ] 서비스($SVCNAME)가 정상적으로 동작에 실패했습니다."
        exit 4 
    fi
}
